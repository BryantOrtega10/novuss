...
class ReportesNominaController extends Controller
{
    ...
    public function documentoNominaHorizontal($idLiquidacionNomina){
        ...
        "ccfijo.valor as valorSalario", "cargo.nombreCargo", "n.nombre as nombreNomina", 
        "emp.razonSocial as nombreEmpresa","bp.fkPeriodoActivo")
        ...
        "cargo.nombreCargo","bp.fkPeriodoActivo")
        ->join("concepto as c","c.idconcepto", "=","ibp.fkConcepto")
        ->join("boucherpago as bp","bp.idBoucherPago","=", "ibp.fkBoucherPago")
        ->join("liquidacionnomina as ln", "ln.idLiquidacionNomina", "=","bp.fkLiquidacion")
        ->join("empleado as e","e.idempleado", "=", "bp.fkEmpleado")
    ...
    public function documentoNominaFechas(Request $req){
        ...
        "dp.primerNombre","dp.segundoNombre", 
        "dp.primerApellido","dp.segundoApellido","ti.nombre as tipoidentificacion", 
        "dp.numeroIdentificacion", "bp.diasTrabajados", "ibp.valor", "bp.idBoucherPago","bp.fkPeriodoActivo","p.fechaInicio as fechaIngreso","p.fechaFin as fechaRetiro", "cargo.nombreCargo", "ibp.cantidad", "ibp.tipoUnidad")
        ->join("concepto as c","c.idconcepto", "=","ibp.fkConcepto")
        ->join("boucherpago as bp","bp.idBoucherPago","=", "ibp.fkBoucherPago")
    ...
    public function documentoNominaHorizontalFechas(Request $req){
        ...
        emp.razonSocial as nom_empresa,n.nombre as nom_nomina, cargo.nombreCargo, cargo2.nombreCargo as nombreCargoPeriodo,
        ...
        limit 0,1) as centroCosto, p.fkNomina as fkNominaPeriodo, e.fkNomina as fkNominaEmpleado")        
        ->join("concepto as c","c.idconcepto", "=","ibp.fkConcepto")
        ->join("boucherpago as bp","bp.idBoucherPago","=", "ibp.fkBoucherPago")
        ->join("liquidacionnomina as ln", "ln.idLiquidacionNomina", "=","bp.fkLiquidacion")
        ->join("empleado as e","e.idempleado", "=", "bp.fkEmpleado")
        ...
        $nominas = $nominas->whereIn("ccfijo.fkConcepto",["1","2","53","54"])
        ...
        for($i = 1; $i<sizeof($reporteFinal) ;$i++){
            ...
            if($existeEmp== -1){
                ...
                foreach($reporteFinal[$i] as $row => $columna){
                    if(is_numeric($columna) && $row!=0 &&$row!=7 && $row!=10 && is_numeric($reporteDatosJuntos[$existeEmp][$row])){
                        try{
    ...
    public function documentoSSArray($idEmpresa, $fechaDocumento){
        ...
        ->select("e.*", "dp.*", "ti.siglaPila","p.fkEstado as estado", "p.fechaInicio as fechaInicioPeriodo"
        ...
        ->where("ln.fkEstado","=","5")
        ->whereRaw("(ln.fechaFin <= '".$fechaFinMesActual."' and ln.fechaInicio >= '".$fechaInicioMesActual."')")    
        //->where("dp.numeroIdentificacion","=","1063955356")
        ->distinct()
        ...
        foreach($empleadosGen as $empleado){
            ...
            if(!isset($ultimoBoucher)){
                ...
                if(strtotime($ultimoBoucher->fechaFin) == strtotime($fechaFinMesActual)){
                    $ultimoBoucherRetiro = DB::table("boucherpago", "bp")
                    ->join("liquidacionnomina as ln","ln.idLiquidacionNomina","=","bp.fkLiquidacion")
                    ->where("bp.fkEmpleado","=",$empleado->idempleado)
                    ->where("bp.fkPeriodoActivo","=",$empleado->idPeriodo)
                    ...
                    //Es una nomina quincenal pero su ultimo boucher normal no es del 30, se tiene que verificar si hay uno de retiro
                    $ultimoBoucherRetiro = DB::table("boucherpago", "bp")
                    ->join("liquidacionnomina as ln","ln.idLiquidacionNomina","=","bp.fkLiquidacion")
                    ->where("bp.fkEmpleado","=",$empleado->idempleado)
                    ->where("bp.fkPeriodoActivo","=",$empleado->idPeriodo)
            ...
            $salarioMes = $conceptoFijoSalario->valor;
            ...
            if($conceptoFijoSalario->valor < $minimosRedondeoFijoSal->ibc){
                $boucherPagoParaCalculoHora = DB::table('boucherpago', 'bp')
                ->selectRaw("sum(bp.periodoPago) as periodoPago, sum(bp.salarioPeriodoPago) as salarioPago")
                ->join("liquidacionnomina as ln","ln.idLiquidacionNomina", "=","bp.fkLiquidacion")
                ->whereIn("ln.fkTipoLiquidacion",["1","2","3","4","5","6","9"]) //1 - Normal, 2- Retiro
                ->where("ln.fkEstado","=","5")
                ->whereRaw("(ln.fechaFin <= '".$fechaFinMesActual."' and ln.fechaInicio >= '".$fechaInicioMesActual."')")    
                ...
                $minimosRedondeoFijoSalParaH = DB::table("tabla_smmlv_redondeo")->where("dias","=",$boucherPagoParaCalculoHora->periodoPago)->first();
                if(isset($minimosRedondeoFijoSalParaH->ibc)){

                    $horas = ($boucherPagoParaCalculoHora->salarioPago * 8)/$minimosRedondeoFijoSalParaH->ibc;
                    $horasTrabajadas = $periodoTrabajado*floor($horas);
    ...
    public function reporteVacaciones(Request $req){
        ...
        if($req->tipoReporte == "PDF"){
            ...
            ->select("dp.numeroIdentificacion", 
                     "c.nombre as nombreConcepto", "a.cantidadDias as ausDias", "a.fechaInicio as ausFechaInicio", "a.fechaFin as ausFechaFin",
                     ...
                     "l.fechaFinal as licFechaFin", "v.diasCompletos as vacDias", "v.fechaInicio as vacFechaInicio", "v.fechaFin as vacFechaFin",
            ...
            ->selectRaw('(select cc2.nombre from centrocosto as cc2 where cc2.idcentroCosto 
                     ...
                     ecc.fkEmpleado = e.idempleado and empleado_centrocosto.fkPeriodoActivo = n.fkPeriodoActivo)
            ...
            ob_start();?>
                ...
                <html>
                    ...
                    foreach($empleados as $row => $empleado){
                        ...
                        if(strtotime($fechaPeriodoCer) < strtotime($empleado->fechaIngreso) && date("Y", strtotime($fechaPeriodoCer)) == date("Y", strtotime($empleado->fechaIngreso))){
    ...
    public function generarNovedades(Request $req){
        $novedades = DB::table("novedad","n")
        ->select("*","a.cantidadDias","c.nombre as nombreConcepto","nom.nombre as nombreNomina", 
    ...
    public function generarFinalReporteador(Request $req){
        ...
        if($reporte->fkTipoReporte == "1"){
            ...
            $consulta = $consulta
            ->joinSub("(
                SELECT    MAX(idCambioSalario) max_id, fechaCambio, valorNuevo,valorAnterior, fkEmpleado 
                FROM      cambiosalario 
                GROUP BY  fkEmpleado
            )","cambio_salario_nuevo","cambio_salario_nuevo.fkEmpleado", "=","e.idempleado","left")
    ...
    public function verificarSiPendientes($idEmpresa, $fecha){
        ...
        if(sizeof($nominasxEmpresa) != sizeof($nominasLiquidadas)){
            return response()->json([
                "success" => false,
                "mensaje" => "Aun cuenta con nominas sin crear, desea continuar?, ".sizeof($nominasxEmpresa)." ".sizeof($nominasLiquidadas)
            ]);
        }
                            ...
                            <?php
                                ...
                                foreach($bouchersPago as $boucherPago){
                                    ...
                                    $empleado = DB::table("empleado","e")
                                        ...
                                        "dp.primerNombre","dp.segundoNombre", 
                                        "dp.primerApellido","dp.segundoApellido","ti.nombre as tipoidentificacion", 
                                        "dp.numeroIdentificacion", "cargo.nombreCargo", "bp.fkPeriodoActivo")
                                        ->join("boucherpago as bp","bp.fkEmpleado", "e.idempleado")
                                        ->join("datospersonales as dp","dp.idDatosPersonales", "=", "e.fkDatosPersonales")
                                        ->join("tipoidentificacion as ti","ti.idtipoIdentificacion", "=", "dp.fkTipoIdentificacion")
                                        ->join("cargo","cargo.idCargo","=","e.fkCargo")
