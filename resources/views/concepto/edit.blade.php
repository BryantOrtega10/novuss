<form method="post" action="/concepto/modificar" class="formGen">
	...
	<div>
        <a href="/concepto/getFormulaConceptoSS/{{$concepto->idconcepto}}" class="modificarFormulaSS">Modificar formula para seguridad social</a>
    </div>
	<div class="respFormulaConceptoSS">
		@if(sizeof($formulaConceptoSS)>0)
		@isset($formulaConceptoSS[0]->fkVariableInicial)
			<input type="hidden" name="fkVariableInicialSS" value="{{$formulaConceptoSS[0]->fkVariableInicial}}" />    
		@endisset
		@isset($formulaConceptoSS[0]->valorInicial)
			<input type="hidden" name="valorInicialSS" value="{{$formulaConceptoSS[0]->valorInicial}}" />
		@endisset
		@isset($formulaConceptoSS[0]->fkGrupoConceptoInicial)
			<input type="hidden" name="grupoInicialSS" value="{{$formulaConceptoSS[0]->fkGrupoConceptoInicial}}" />
		@endisset
		@isset($formulaConceptoSS[0]->fkConceptoInicial)
			<input type="hidden" name="conceptoInicialSS" value="{{$formulaConceptoSS[0]->fkConceptoInicial}}" />
		@endisset

		@for ($i = 0; $i < sizeof($formulaConceptoSS); $i++)

			<input type="hidden" name="fkTipoOperacionSS[]" value="{{$formulaConceptoSS[$i]->fkTipoOperacion}}" />
			<input type="hidden" name="fkVariableFinalSS[]" value="{{$formulaConceptoSS[$i]->fkVariableFinal}}" />
			<input type="hidden" name="valorFinalSS[]" value="{{$formulaConceptoSS[$i]->valorFinal}}" />
			<input type="hidden" name="grupoFinalSS[]" value="{{$formulaConceptoSS[$i]->fkGrupoConceptoFinal}}" />
			<input type="hidden" name="conceptoFinalSS[]" value="{{$formulaConceptoSS[$i]->fkConceptoFinal}}" />
		@endfor
	@endif
	...
	<input type="hidden" class="form-control" id="numRetefuente" name="numRetefuente" value="{{$concepto->numRetefuente}}" />
	<div class="form-group">
	...
	<button type="submit" class="btn btn-success">Modificar concepto</button>
</form>